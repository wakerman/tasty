	...
	<div id="container">
        ...
        <div id="wrapper">
			<div id="content">
       			<div id="box">
                	<h3>DataId下的发布者数</h3>
                    <form id="form" class="expose" action="${baseUrl}/data/dataid_publishers.html" method="get">                    	
            ...
            <c:if test="${not empty dataId}">
	            <c:if test="${not empty machines}">
					<div id="result" class="tips">发布表达式[${dataId}]的发布者的机器总数为：${machines}</div>
				</c:if>
			</c:if>
			<c:if test="${empty dataId}">
	            <c:if test="${not empty machines}">
					<div id="result" class="tips">所有发布者的机器总数为：${machines}</div>
				</c:if>
			</c:if>
           <c:choose>
